...
export default function StateModal({
  ...
  const saveState = async (values) => {
    try {
      ...
      console.log(error);
      Alert.alert("Mistake", error.message);
    }
  };
  ...
  return (
    <Base
      id="modal-state"
      visible={visible}
      title={selected?.key ? "Edit state" : "Create state"}
      onClose={onClose}
    >
      <FormItem
        value={selected?.name || ""}
      ...
      <SelectDropdown
        ...
        defaultButtonText={"Select the status"}
      ...
      />
      <Button
        label={selected?.key ? "EDIT" : "TO REGISTER"}
        onPress={saveState}
        isLoading={loading}
      />
    </Base>
